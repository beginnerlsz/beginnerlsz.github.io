...
- 利用bp插件[BypassPro](https://github.com/0x727/BypassPro)

### 7. 带审核的站点测试方法

- 对于有的站点，注册提交信息后需要审核的，提交信息完以后可以去把接口重新测一遍，有的接口权限设置可能不是很完善，就有可能存在越权

  登录后的站点，发现如下场景（*任何可能导致账号角色发生变化的场景*）,都可以去重新测一下所有接口
  ...
  - 信息注册
    ...
    ```tex
    比如一个招聘网站，存在招聘者，应聘者，当应聘者提交信息成为待审核的应聘者时，此时应聘者不应该拥有给企业内部员工的权限，但是当权限设置不完善的时候，待审核的应聘者就有可能拥有部分权限，从而实现越权
    ```
...
- 卡住数据包不放
  ...
  > 卡住鉴权包以后将页面上可以用的功能全部点一遍，然后放包，这样点击过的功能都会有请求包发出
...
1. 数据包如下
   ...
   ```http
   POST /6161616/18496716/16481/_21964161
   host:
   
   method=POST&data=ACC_admin_getuserList&body={"pageno":1,"pageSize":1}
   ```
   ...
   functionid=getUserinfoByid&body={"id":1}&header=POST
   ```

   **说明**：接口的具体功能由`data`或者`functionid`这两个参数决定，请求方式由`method`决定，`body` 里面是请求参数。

   **测试方法**：去js中搜索`getUserinfoByid`,这样可能会带出很多functionId。
